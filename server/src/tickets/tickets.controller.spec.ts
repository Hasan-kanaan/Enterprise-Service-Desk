...
describe('TicketsController', () => {
  ...
  const controller = new TicketsController({ listVisible, findVisibleById } as any);
  ...
  it('lists tickets through the visibility policy', async () => {
    ...
    expect(listVisible).toHaveBeenCalledWith({ id: 10, role: UserRole.EMPLOYEE });
  });

  it('looks up ticket details through the visibility policy', async () => {
    findVisibleById.mockResolvedValue({ id: 12 });

    await controller.findById(12, { user: { sub: 31, role: UserRole.MANAGER } });
  ...
  it('preserves generic not-found behavior from the visibility policy', async () => {
    findVisibleById.mockRejectedValue(new NotFoundException('Ticket not found'));
  ...
  it('preserves ADMIN denial from the visibility policy', async () => {
    listVisible.mockRejectedValue(new ForbiddenException('ADMIN users do not have ticket visibility'));
